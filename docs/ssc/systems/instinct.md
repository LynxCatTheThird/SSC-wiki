...
本能值（Instinct）是常规形态的长期阶段推进机制。它从 `0` 开始，达到 `100` 时尝试将玩家变为同一形态组的下一阶段，随后清零。[^source]

## 基础增长

养本能值的目标，是让当前形态更快达到下一阶段。最稳妥的做法是按形态页给出的环境和食物行动，持续待在有利地点；离开适宜环境、吃错食物或触发负面行为会拖慢进度。第二阶段通常应先把本能养满，再准备共鸣催化剂进入永久阶段。
...
| 使魔红狐 | 待在针叶林、浆果丛或雪上移动，食用浆果，观察女巫或唤魔者 |
| 雪狐     | 冲刺跳跃；其他倾向以形态页和手册为准                     |
| 胡狼     | 待在沙漠、食用腐肉、承受凋零效果                         |
| 蜘蛛     | 使用结露蛛网和茧阶段的专用流程，不应套用普通阶段路线     |

手册中的 “本能” 文字是玩家提示；真正改变数值的是形态加载的能力及其条件。进入有利环境后观察本能条是否持续上升，就能确认行为是否生效。整合包若覆盖 Origin JSON，也可能改变这些规则。
...
第二阶段常用于锁定普通本能推进，永久阶段需要共鸣催化剂等专门条件。蜘蛛茧和特殊形态另有自己的变身规则。
...
[^source]: 源码核对：幻形者诅咒 `InstinctUtils.java`、`StaticParams.java`、`FormUtils.java`、各形态 Origin 与 instinct power JSON，commit `c0f0bbb9`。系统规则已核对；尚未发现数值定义的行为不会据此推测。

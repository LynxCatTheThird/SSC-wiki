...
悦灵形态和野猫形态各自只有一个阶段，没有本能阶段，也不会受诅咒之月推进。两者都属于可恢复的特殊形态：喝普通抑制剂即可回到原始变形者。它们也是幻形者诅咒扩展包对应两组终局分支的起点。[^source]
...
使用悦灵药水取得转化效果后睡觉或食用催化剂即可进入；普通抑制剂可以恢复原始变形者。
...
基础 20 点生命减少 10 点，只剩 10 点，也就是 5 颗心。单次攻击造成至少 4 点伤害时，自身恢复 2 点生命，内部冷却 10 tick，也就是 0.5 秒。

#### 挖掘

硬度至少为 1 的方块挖掘速度降低 45%。

#### 近战与远程攻击

投射物伤害固定增加 6 点。使用挖掘等级高于石质的主手工具攻击时，总伤害降低 33%。

#### 饥饿

普通食物无法食用，紫水晶碎片和模组允许的特殊食物例外。飞行时每 5 tick 增加 1.333 点饥饿消耗。

#### 移动与飞行

按住跳跃键可以上升，单次最多持续 80 tick，也就是 4 秒；飞行时移动速度提高 16%。悦灵形态可以走过细雪，并拥有缓降与落地免疫。

#### 音乐互动

距离正在播放唱片的唱片机不超过 3 格时，每 40 tick，也就是每 2 秒恢复 2 点生命。
...
| 名称     | 命名空间 ID | 本地化键名                     |
| -------- | ----------- | ------------------------------ |
| 悦灵形态 | `allay_sp`  | `shape-shifter-curse.allay_sp` |
...
使用野猫药水取得转化效果后睡觉或食用催化剂即可进入；普通抑制剂可以恢复原始变形者。
...
野猫形态是四足潜行形态，沿用豹猫形态终局阶段的主要移动能力。它可以全向攀爬，免疫摔落，脚步无声且不会触发绊线，苦力怕会主动远离；猫与豹猫生物把玩家视为友好目标。它还有猫科夜视和四足视角摆动。

#### 阵营与环境

猫与豹猫生物把玩家视为友好目标，苦力怕会主动远离。安装 Tough As Nails 时，野猫形态额外获得轻度寒冷抗性与脏水口渴保护；未安装该模组时，这两项兼容能力不会产生 Minecraft 自带机制之外的环境温度系统。
...
| 名称     | 命名空间 ID | 本地化键名                         |
| -------- | ----------- | ---------------------------------- |
| 野猫形态 | `feral_cat_sp` | `shape-shifter-curse.feral_cat_sp` |
...
安装幻形者诅咒扩展包后，悦灵形态和野猫形态都有两条终局分支。全部目标集中列在 [幻形者诅咒扩展包形态目录](../../../addons/ssca/forms/overview)，进化道具、月相条件和失败后果见 [进化系统](../../../addons/ssca/systems/evolution)。

...
第一次变身不需要等待诅咒之月。正常流程是：找到带有附魔粒子的异常生物，让它攻击并取得对应的转化效果，再在效果消失前睡觉。
...
SSC 1.10.0 注册了五种带有附魔粒子的转化生物。它们不是普通生物换皮，而是独立实体：
...
:::note 狐形态不是这条自然感染路径
当前源码中的咒文豺狼灵只给予阿努比斯之狼效果。使魔狐、雪狐及特殊形态虽然注册了转化效果，但不能据此推断都有对应的自然生成实体；它们的具体获取路线以各形态页和游戏内手册为准。
:::
...
- 第 4 阶段通常是永久性最终形态，使用共鸣催化剂前应先阅读对应形态页。
...
来源：SSC `give_book`/`delay_give_book` 函数、`TransformativeEntitySpawning.java`、`ITMob.java`、`EffectManager.java`、`TransformativeStatusInstance.java`，commit `c0f0bbb9`。状态：`verified`（代码路径）；实际生成密度和跨模组睡眠兼容性：`partial`。

...
其中 `t` 是游戏 tick 数，`r_k` 是该 tick 的环境或行为修正。诅咒之月期间，增长率直接变为 `0`。
...
本能条按 `0` 到 `100` 显示，并根据当前净增长率使用不同状态。数值达到 `80`、但尚未接近 `100` 时，客户端会在玩家附近显示附魔粒子，提示变化正在逼近。

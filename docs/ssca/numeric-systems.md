...
范围技能常见球形、圆柱形和轴对齐方盒三种判定。半径 20 格的球形技能会检查三维直线距离；高度 9 格、半径 24 格的圆柱领域会分别限制水平距离和上下高度；$3\times3\times3$ 方盒则覆盖施法者周围固定长宽高。相同“20 格”文字在不同形状下覆盖体积会明显不同。
...
| --- | --- | ---: | --- | --- |
| SP 悦灵 | Mana，$0\le R\le200$ | 0 | 每 8 tick 恢复 3；远程命中每 3.5 秒可恢复上限的 8% | 群疗至少需要 100，完成后清空；信标与应急食物也会消耗 |
| 吸血蝙蝠 | 血渴，$0\le R\le100$ | 0 | 普攻命中 $+8$，技能前三个目标依次 $+12/+6/+3$，击杀 $+15$ | 数值区间直接改变减伤、吸血和伤害 |
| 冥裁者 | 灵魂能量，$0\le R\le100$ | 0 | 普通击杀 $+5$，冥狼击杀 $+10$，领域内击杀 $+20$，自身凋零时额外 $+10$ | 满 100 强化下一次死亡领域并清空 |
| SP 雪狐 | 霜寒，$0\le R\le100$ | 0 | 技能或受伤后暂停 5 秒；随后每秒恢复 3，适宜温度环境恢复 5 | 近战主技能 15，瞬移成功 30、失败 20，冰风暴 30，护盾触发 20 |
| 寄生果蝠 | 种子量，$0\le R\le10$ | 10 | 脱战每 5 秒 $+1$，交战每 8 秒 $+1$；吃种子也可 $+1$ | 灵果寄生与孢子弹每次消耗 1 |
...
| 资源 | 条件 | 从空到满的理论时间 |
| --- | --- | ---: |
| SP 悦灵 Mana | 每秒 7.5 点 | 约 26.7 秒 |
| SP 雪狐霜寒 | 普通环境每秒 3 点 | 约 33.3 秒 |
| SP 雪狐霜寒 | 适宜环境每秒 5 点 | 20 秒 |
| 寄生果蝠种子量 | 脱战每 5 秒 1 点 | 50 秒 |
| 寄生果蝠种子量 | 交战每 8 秒 1 点 | 80 秒 |
...
同一能力中的加法、总乘区和最终倍率可能处于不同计算阶段。两个 $+20\%$ 效果有时得到 $+40\%$，有时会连乘成 $1.2\times1.2=1.44$。本站只在源码能确认运算位置时给出合并后的结果；单个效果仍按自身数值陈述。

来源：SSCA `ResourceBars.java`、`BarKeys.java`、各资源 power JSON、`BatDesmodusBloodThirst.java`、`ParasiticSeedEnergyRegen.java`、`NineLivesManager.java` 与相关技能类，commit `d84a18ca`。状态：统一资源规则与表中基础值 `verified`；饰品叠加和跨模组伤害链 `partial`。

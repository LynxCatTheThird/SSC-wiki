...
title: 命令参考
sidebar_position: 1
description: 幻形者诅咒与幻形者诅咒扩展包 1.20.1 当前命令、参数和权限
---
...
形态参数使用幻形者诅咒自定义参数类型并提供自动补全。内置、动态和子形态分别过滤，不要把页面显示名当作 ID。
...
| 命令                              | 权限 | 说明                                                                 |
| --------------------------------- | ---: | -------------------------------------------------------------------- |
| `set_mana <targets> <amount>`     |    2 | 设置目标当前拥有的幻形者诅咒扩展包主资源；最小值 0，超过上限时按该资源上限写入 |
...
4.幻形者诅咒扩展包命令缺失时，先确认幻形者诅咒扩展包是否因幻形者诅咒版本不匹配而加载失败。
5. 数据修复前备份世界目录。

[^source]: 源码核对：幻形者诅咒 `ShapeShifterCurseCommand.java`、`FormArgumentType.java`；幻形者诅咒扩展包 `SscAddonCommands.java`，分别对应 commit `c0f0bbb9`、`e42a62aa`。命令树与参数已经逐项核对；权限插件仍可能改变服务器上的实际可用范围。

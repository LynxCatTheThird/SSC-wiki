...
这五项控制符合地点条件的咒文生物通过一次生成尝试的机会。数值只影响生成环节，对感染成功率没有影响。填 `0` 会关闭对应生物的自然生成，填 `1` 会让每次合法尝试都通过。
...
| ------------------------------ | -------------- | ------------------------------------- | ---------------------------------------------- |
| 第一人称能否看到形态模型       | 显示           | 关闭后保留未变形时的第一人称视角      | `enableFormModelOnVanillaFirstPersonRender`    |
...
Instinct、魔力值和物品存储条使用 1–9 九宫格锚点加 X/Y 偏移。默认锚点均为 `8`：
...
| 契灵形态瞬移落点             | 朝准星方向选择 | 平台模式会按另一套落点规则寻找位置     | `mancianimaTeleportMode` |

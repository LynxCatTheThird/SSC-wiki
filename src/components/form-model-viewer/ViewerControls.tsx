...
export default function ViewerControls({
  ...
  return <>
    <div className="form-model-viewer__controls">
      ...
      <button type="button" disabled={!ready} onClick={onSettingsToggle} aria-expanded={settingsOpen}>参数</button>
    </div>

    {settingsOpen && camera && homeCamera && <CameraSettings
      camera={camera}
...
function CameraSettings({camera, homeCamera, appearance, onCameraChange, onAppearanceChange, onReset}: {
  ...
  return <div className="form-model-viewer__settings">
    <fieldset>
      <legend>画幅位置</legend>
      <Range label="水平位置 X" value={camera.targetX} {...targetX} step={0.01} unit="m" digits={2} onChange={(value) => onCameraChange('targetX', value)} />
      <Range label="垂直位置 Y" value={camera.targetY} {...targetY} step={0.01} unit="m" digits={2} onChange={(value) => onCameraChange('targetY', value)} />
      <Range label="纵深位置 Z" value={camera.targetZ} {...targetZ} step={0.01} unit="m" digits={2} onChange={(value) => onCameraChange('targetZ', value)} />
    </fieldset>
    <fieldset>
      <legend>观察角度</legend>
      <Range label="水平角度" value={camera.orbitX} min={-180} max={180} step={1} unit="°" onChange={(value) => onCameraChange('orbitX', value)} />
      <Range label="俯仰角度" value={camera.orbitY} min={5} max={175} step={1} unit="°" onChange={(value) => onCameraChange('orbitY', value)} />
    </fieldset>
    <fieldset>
      <legend>镜头</legend>
      <Range label="镜头距离 R" value={camera.distance} min={distanceMin} max={distanceMax} step={0.01} unit="m" digits={2} onChange={(value) => onCameraChange('distance', value)} />
      <Range label="镜头视野" value={camera.fieldOfView} min={12} max={55} step={1} unit="°" onChange={(value) => onCameraChange('fieldOfView', value)} />
    </fieldset>
    <fieldset>
      <legend>光照与运动</legend>
      <Range label="曝光" value={appearance.exposure} min={0.25} max={2} step={0.05} digits={2} onChange={(value) => onAppearanceChange('exposure', value)} />
      <Range label="阴影浓度" value={appearance.shadowIntensity} min={0} max={1} step={0.05} digits={2} onChange={(value) => onAppearanceChange('shadowIntensity', value)} />
      <Range label="阴影柔度" value={appearance.shadowSoftness} min={0} max={1} step={0.05} digits={2} onChange={(value) => onAppearanceChange('shadowSoftness', value)} />
      <Range label="旋转速度" value={appearance.rotationSpeed} min={4} max={60} step={1} unit="°/s" onChange={(value) => onAppearanceChange('rotationSpeed', value)} />
    </fieldset>
    <div className="form-model-viewer__settings-actions">
      <button type="button" onClick={onReset}>恢复当前阶段默认视图</button>
    </div>
  </div>;
}
...
function Range({label, value, min, max, step, unit = '', digits = 0, onChange}: {
  ...
  return <label className="form-model-viewer__range">
    <span>{label}</span>

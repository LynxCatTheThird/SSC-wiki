...
export default function FormModelViewer({
  ...
  const bridge = useModelViewerBridge({
    ...
    onLoad: (generation, camera, availableAnimations, animation, playing) => dispatch({
      type: 'MODEL_LOADED',
      generation,
      camera,
  ...
  useEffect(() => {
    ...
    void ensureModelViewerDefined()
      .then(() => { if (!cancelled) dispatch({type: 'REGISTERED'}); })
      .catch(() => { if (!cancelled) dispatch({type: 'REGISTRATION_FAILED'}); });
  ...
  useEffect(() => {
    ...
    void bridge.loadStage(activeStage, state.generation)
  ...
  return <figure className="form-model-viewer">
    <div className="form-model-viewer__stage" aria-busy={state.phase === 'registering' || state.phase === 'loading'}>
      {state.moduleReady && activeStage && <model-viewer
      ...
      />}
      {state.phase !== 'ready' && <div className="form-model-viewer__fallback" role="status">
